...
int main(int argc, char *argv[]) {
    ...
    if (fractionSize < 2 || fractionSize > 10) {
        printf("Illegal number of fraction bits. Should be between 2 and 10.\n");
        exit(0);
    }
    ...
    exponent = (unsigned) exponent >> fractionSize;
    if (normalized) {
    ...
    if (exponent == pow(2, exponentSize) - 1) {
        if (!fraction) {
            if (signedBit) {
                printf("-Infinity.\n\n");
                exit(0);
